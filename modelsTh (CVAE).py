...
class VAE():
    ...
    def __init__(self, source_shape, target_shape, build_encoder_fn, build_decoder_fn,
        ...
        with tf.compat.v1.variable_scope("vae", reuse=reuse):
            ...
            self.mean      = tf.compat.v1.layers.dense(encoded, z_dim, activation=None, name="mean")
            self.logstd_sq = tf.compat.v1.layers.dense(encoded, z_dim, activation=None, name="logstd_sqare")
            ...
            # Reconstruct image
            self.reconstructed_logits = tf.compat.v1.layers.flatten(decoded, name="reconstructed_logits")
            ...
            if training:
                # Reconstruction loss
                self.flattened_target = tf.compat.v1.layers.flatten(target_states, name="flattened_target")
                ...
                self.mean_reconstruction_loss, self.update_mean_reconstruction_loss = tf.compat.v1.metrics.mean(self.reconstruction_loss)
...
class ConvVAE(VAE):
    ...
    def __init__(self, source_shape, target_shape=None, **kwargs):
        ...
        def build_encoder(x):
            x = tf.compat.v1.layers.conv2d(x, filters=32,  kernel_size=4, strides=2, activation=tf.nn.relu, padding="valid", name="conv1")
            x = tf.compat.v1.layers.conv2d(x, filters=64,  kernel_size=4, strides=2, activation=tf.nn.relu, padding="valid", name="conv2")
            x = tf.compat.v1.layers.conv2d(x, filters=128, kernel_size=4, strides=2, activation=tf.nn.relu, padding="valid", name="conv3")
            x = tf.compat.v1.layers.conv2d(x, filters=256, kernel_size=4, strides=2, activation=tf.nn.relu, padding="valid", name="conv4")
            self.encoded_shape = x.shape[1:]
            x = tf.compat.v1.layers.flatten(x, name="flatten")
            return x

        def build_decoder(z):
            x = tf.compat.v1.layers.dense(z, np.prod(self.encoded_shape), activation=None, name="dense1")
            x = tf.reshape(x, (-1, *self.encoded_shape))
            x = tf.compat.v1.layers.conv2d_transpose(x, filters=128, kernel_size=4, strides=2, activation=tf.nn.relu, padding="valid", name="deconv1")
            x = tf.compat.v1.layers.conv2d_transpose(x, filters=64,  kernel_size=4, strides=2, activation=tf.nn.relu, padding="valid", name="deconv2")
            x = tf.compat.v1.layers.conv2d_transpose(x, filters=32,  kernel_size=5, strides=2, activation=tf.nn.relu, padding="valid", name="deconv3")
            x = tf.compat.v1.layers.conv2d_transpose(x, filters=target_shape[-1], kernel_size=4, strides=2, activation=None, padding="valid", name="deconv4")
...
class MlpVAE(VAE):
    ...
    def __init__(self, source_shape, target_shape=None,
        ...
        def build_encoder(x):
            x = tf.compat.v1.layers.flatten(x, name="flattened_source")
