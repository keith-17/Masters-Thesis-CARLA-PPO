...
class PolicyGraph():
    ...
    def __init__(self, input_states, taken_actions, action_space, scope_name,
                 ...
                 pi_hidden_sizes=(500, 300), vf_hidden_sizes=(500, 300), memory=None):
        """
        ...
        with tf.variable_scope(scope_name):
            ...
            self.action_mean = tf.layers.dense(self.pi, num_actions,
                                               ...
                                               name="action_mean")
            #print(self.action_mean,"OVER HEREEE")
    ...
    def build_mlp(self, x, hidden_sizes=(32,), activation=tf.tanh, output_activation=None):
        ...
        for h in hidden_sizes[:-1]:
            ...
            #x = tf.keras.layers.SimpleRNNCell(units=h)(x)
            #rah+=1
            #print(rah)
            #print(h)
...
#h is states
    ...
    def build_lstm(self, x, hidden_sizes=(32,), activation=tf.tanh, output_activation=None):
        ...
        x = tf.layers.dense(x, units=500, activation=activation)
            #x = tf.compat.v1.keras.layers.CuDNNLSTM(units=h)(x)
            #x = tf.layers.dense(x, units=h, activation=activation) #tf.compat.v1.keras.layers.CuDNNLSTM(units=hidden_sizes[-1])(x)
            #x = tf.keras.layers.SimpleRNN(units=h)(x)
            #x = tf.keras.layers.GRU(units=h)(x)
        #x = tf.keras.layers.SimpleRNN(units=500)(x)
        #x = tf.compat.v1.keras.layers.CuDNNLSTM(units=3)(x)
        #x = tf.layers.dense(x, units=500, activation=activation)
        #x = tf.compat.v1.keras.layers.CuDNNLSTM(units=3)(x)
        #x = tf.layers.dense(x, units=500, activation=activation)
        #x = tf.layers.dense(x, units=500, activation=activation)
        #x = tf.layers.dense(x, units=500, activation=activation)
        #x = tf.keras.layers.Conv2D(units=h, strides=(3,3) input_shape=(80, 160, 4))(x)

        return x
        #return tf.keras.layers.SimpleRNN(units=hidden_sizes[-1])(x)
...
class PPO():
    ...
    def __init__(self, input_shape, action_space,
        ...
        self.policy        = PolicyGraph(self.input_states, self.taken_actions, action_space, "policy", initial_std=initial_std, pi_hidden_sizes=pi_hidden_sizes, memory=self.memory)
        self.policy_old    = PolicyGraph(self.input_states, self.taken_actions, action_space, "policy_old", initial_std=initial_std, pi_hidden_sizes=pi_hidden_sizes, memory=self.memory)
    ...
    def predict(self, input_states, greedy=False, write_to_summary=False):
        ...
        sampled_action, value, summaries, step_idx = \
            self.sess.run([action, self.policy.value, self.stepwise_prediction_summaries, self.predict_step_counter.var],
                feed_dict={self.input_states: input_states, self.memory: memory}
            )
